window.onload = function () {
    ...
    axios.get(`http://localhost:3005/expense/getexpense?rpp=${Rows_perpage}`, { headers: {"Authorization": token}})
        .then(res => {
...
function getproducts(page) {
    ...
    axios.get(`http://localhost:3005/expense/getexpense?paaag=${page}&rpp=${Row_perpage}`, { headers: {"Authorization": token}})
        .then(res => {
            showOldExpensesonScreen(res.data.Expenses);
            ShowingPagination(res.data);
        })
...
function addingExpense(event) {
    ...
    axios.post('http://localhost:3005/expense/addexpense', { key1: expenseAmount, key2: description, key3: category },{ headers: {"Authorization": token}})
        .then(res => {
...
function deleteexpenseLOCAL(f) {
    axios.delete(`http://localhost:3005/expense/deleting/${f}`)
        .then(result => {
            console.log('hello')
        })
        .catch(err => { console.log(err) });
