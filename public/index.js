function addingUser(event) {
  ...
  axios.post('http://localhost:3005/user/SigningUp', { key1: name, key2: email, key3: password })
    .then(result => {
...
function logInUser(event) {
  ...
  axios.post('http://localhost:3005/user/login', { key1: email, key2: password })
    .then((result) => {
...
function resetPassword(event){
  ...
  const email=event.target.email.value;
  // console.log(email);
    axios.post('http://localhost:3005/password/forgotpassword', {key1: email})
